function [ppoints, opttour, clustermst, totcos,Cnwlftime50, Cnwlftime90, Cnwlftime100] = optour(M,X,Y,CR)
...
G = sparse(clustermat);
[clustermst]=graphminspantree(G);
minst = full(clustermst);
...
for i = 1:n-1
    for j = i+1:n
        ...
        if i < j && j == i+1
            ...
            for k = 1 : M
                for l = 1 : M
                    if C(k) == opttour(i) && C(l) == opttour(j) && opttour(i) ~= opttour(j)
                        ...
                        if temp <= min
                            min = temp;
                            u = k;
                            firstcluster = opttour(i);
                            secondcluster = opttour(j);
                            v = l;
                        end
...
totcos = int32(cost)+int32(Tourlength);
% disp('total number of polling points are');

% network lifetime for network

vorder = [];
for i = 1:numel(opttour)
    for k = 1:numel(ppoints)
        
            if C(ppoints(k)) == opttour(i)
                vorder = [vorder,ppoints(k)];
            end
    
    end
end
[ Cnwlftime50, Cnwlftime90, Cnwlftime100] = Networklifetime(M, X, Y, ppoints, Nmsg, vorder);
